...
CHROME_DVR_DIR = '/usr/lib/chromium-browser/chromedriver'
    ...
    while True:
        try:
            for line in f:
                browser.get(website)
                t.sleep(2)
                Sel_user = browser.find_element_by_id(username_selector) #Finds Selector
                Sel_pas = browser.find_element_by_id(password_selector) #Finds Selector
                enter = browser.find_element_by_id(login_btn_selector) #Finds Selector
                # browser.find_element_by_id(password_selector).clear()
                # browser.find_element_by_id(username_selector).clear()
